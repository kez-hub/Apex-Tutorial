...
exports.onPaymentCompleted = exports.cleanupOldNotifications = exports.notifyStudentsOnNewCourse = exports.notesPdfProxy = void 0;
...
async function requirePaidOrInstructor(uid) {
    ...
    if (role === "instructor" || data?.isAdmin === true)
        return;
    if (role === "student" && hasPaid)
        return;
    throw new functions.https.HttpsError("permission-denied", "Payment required");
}
function parseStoragePathFromUrl(pdfUrl) {
    try {
 ...
 */
exports.notesPdfProxy = functions.https.onRequest(async (req, res) => {
    ...
    try {
        ...
        const objectPath = pathFromQuery ||
 ...
 */
exports.notifyStudentsOnNewCourse = functions.firestore
    .document("courses/{courseId}")
    .onCreate(async (snap, context) => {
    try {
        const courseData = snap.data();
        const courseId = context.params.courseId;
        console.log(`New course created: ${courseData.title} by ${courseData.instructorName}`);
        ...
        studentsSnapshot.docs.forEach((studentDoc) => {
            ...
            const notificationRef = db.doc(`notifications/${studentId}/${notificationId}`);
        ...
        await batch.commit();
        console.log(`Successfully created notifications for ${studentsSnapshot.size} students`);
        return {
            success: true,
...
exports.cleanupOldNotifications = functions.pubsub
    ...
    try {
        ...
        console.log(`Cleanup completed. Deleted ${totalDeleted} old notifications`);
